...
from anvil.models.mod_list_model import ModListModel, COL_CHECK, COL_NAME, ROLE_IS_SEPARATOR, ROLE_FOLDER_NAME, ROLE_SEP_COLOR, ROLE_IS_LOCKED, ROLE_GROUP_NAME, ROLE_IS_GROUP_HEAD
...
class GroupNameDelegate(QStyledItemDelegate):
    ...
    def paint(self, painter: QPainter, option, index):
        ...
        if is_head:
            ...
            # If collapsed, append member count (Kriterium 2)
...
class ModListProxyModel(QSortFilterProxyModel):
    ...
    def filterAcceptsRow(self, source_row, source_parent):
        ...
        has_filter = self._filter_text or self._filter_prop_ids or self._filter_cat_ids
        if not has_filter:
            return True
        ...
        if source:
            ...
            # Group-head: visible if ANY member passes the filter (Kriterium 10)
            if source.data(idx, ROLE_IS_GROUP_HEAD) and self._group_manager:
                ...
                if group_name:
                    ...
                    for member_folder in members:
                        # Find the source row for this member
