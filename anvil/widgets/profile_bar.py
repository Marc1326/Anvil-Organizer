...
class ProfileBar(QWidget):
    ...
    export_anvilpack_requested = Signal()
    import_csv_requested = Signal()
    import_anvilpack_requested = Signal()
    ...
    def __init__(self, parent=None):
        ...
        menu1.addSeparator()
        export_menu = menu1.addMenu(tr("export_import.export_radio"))
        export_menu.addAction(QAction(tr("export_import.csv_radio"), self, triggered=lambda checked: self.export_csv_requested.emit()))
        export_menu.addAction(QAction(tr("export_import.anvilpack_radio"), self, triggered=lambda checked: self.export_anvilpack_requested.emit()))
        import_menu = menu1.addMenu(tr("export_import.import_radio"))
        import_menu.addAction(QAction(tr("export_import.csv_radio"), self, triggered=lambda checked: self.import_csv_requested.emit()))
        import_menu.addAction(QAction(tr("export_import.anvilpack_radio"), self, triggered=lambda checked: self.import_anvilpack_requested.emit()))
        menu1.addSeparator()
        menu1.addAction(QAction(tr("export_import.backup_create"), self, triggered=lambda checked: self.backup_requested.emit()))
        menu1.addAction(QAction(tr("export_import.backup_restore"), self, triggered=lambda checked: self.restore_requested.emit()))
